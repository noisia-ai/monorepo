...
function buildSignalMentionPredicateFromValidatedFilterV1(
  ...
  if (textSearch) {
    ...
    const searchParameter = parameter(`%${escapeLikePattern(textSearch)}%`);
    conditions.push(`(
      to_tsvector(
        'simple',
        concat_ws(' ', COALESCE(m.title, ''), COALESCE(m.text_clean, ''), COALESCE(m.text_snippet, ''))
      ) @@ websearch_to_tsquery('simple', ${fullTextParameter})
      OR concat_ws(
        ' ',
        NULLIF(m.title, ''),
        NULLIF(m.text_clean, ''),
...
function fixtureMatchesFilter(mention: SignalFixtureMentionV1, filter: SignalFilterV1) {
  ...
  if (
    textSearch
    && !(mention.text ?? "").normalize("NFC").toLocaleLowerCase("en-US").includes(textSearch)
  ) return false;
