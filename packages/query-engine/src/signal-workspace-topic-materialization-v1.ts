...
export function mergeSignalWorkspaceTopicMaterializationV1(args: {
  ...
  execution_id: string; now: string; locale: "es-MX" | "en-US";
  ...
  if (!z.string().uuid().safeParse(args.execution_id).success || !z.string().datetime().safeParse(args.now).success
    || !["es-MX", "en-US"].includes(args.locale)) throw new Error("workspace_engine_materialization_context_invalid");
  ...
  for (const proposal of args.interpretations) {
    ...
    if (!definition) {
      ...
      const fallback = args.locale === "es-MX"
        ? { label: "Grupo sin interpretación suficiente", definition: "La evidencia disponible todavía no permite definir esta conversación." }
        : { label: "Group with insufficient interpretation", definition: "The available evidence does not yet support a definition of this conversation." };
      const draft = {
