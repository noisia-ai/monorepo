...
export function AcquisitionPlanManager({
  ...
  const canConfigure = !importsOnly || canProcess;
  const [configurationOpen,setShowConfiguration] = useState(!importsOnly);
  const showConfiguration = canConfigure && configurationOpen;
  ...
  const loadState = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if(canConfigure && planPayload.draft_plan){
  ...
  },[canConfigure,planLoadError,sourceLoadError,briefLoadError,scopedFetch,workspaceId]);
  ...
  useEffect(() => { if (canConfigure && configurationRequest > 0) setShowConfiguration(true); }, [canConfigure, configurationRequest]);
  ...
  return <>
    <AdminResourceSection
      actions={<div className="admin-workspace-actions">
        ...
        {importsOnly && canConfigure ? <button className="admin-button" onClick={() => setShowConfiguration((value) => !value)} type="button">{t("manualImport.configuration")}</button> : null}
    ...
    >
      ...
      {!loading&&plan?<>
        ...
        {slotViews.length===0?<div className="admin-empty"><Target aria-hidden size={24}/><strong>{t("empty.title")}</strong><p>{t("empty.body")}</p>{canConfigure ? <button className="admin-button admin-button--primary" onClick={()=>void reconcile()} type="button">{t("actions.prepare")}</button> : null}</div>:<div className={`admin-acquisition__slots${importsOnly && !showConfiguration ? " admin-acquisition__slots--import" : ""}`}>
    ...
    {canConfigure && drawer?.mode==="review"?<WorkspaceDrawer ariaLabel={t("drawers.review.title",{slot:drawer.slot.label})} closeLabel={common("actions.close")} eyebrow={`${t(`scopes.${drawer.slot.scope}`)} · ${drawer.slot.label}`} onClose={closeDrawer} title={t("drawers.review.title",{slot:drawer.slot.label})}>
...
export function ImportForm({canCancelUpload,onCancelUpload,onRefreshStatus,simple=false,busy,connectors,error,locale,onRegisterQuery,onSubmit,queries,readyForImport,result,t,timezone}:{
  ...
  return <form className="admin-drawer-form" onSubmit={(event)=>{event.preventDefault();onSubmit(new FormData(event.currentTarget));}}>
    ...
    <details open={!simple}><summary>{t(simple ? "manualImport.optionalQuery" : "queryEvidence.title")}</summary><fieldset className="workspace-field admin-acquisition__evidence"><legend className="sr-only">{t("queryEvidence.title")}</legend>
      ...
      {onRegisterQuery ? <button className="admin-button admin-button--compact" onClick={onRegisterQuery} type="button"><PencilSimple aria-hidden size={14}/>{t("queryEvidence.registerExecuted")}</button> : null}
