...
export function AcquisitionPlanManager({
  ...
  return <>
    ...
    {drawer?.mode==="review"?<WorkspaceDrawer ariaLabel={t("drawers.review.title",{slot:drawer.slot.label})} closeLabel={common("actions.close")} eyebrow={`${t(`scopes.${drawer.slot.scope}`)} · ${drawer.slot.label}`} onClose={closeDrawer} title={t("drawers.review.title",{slot:drawer.slot.label})}>
      ...
      {queryDetail?<QueryReview detail={queryDetail} error={error} locale={locale} onAdvancedEdit={()=>setDrawer({mode:"query",slot:drawer.slot})} onSubmit={(form)=>void reviewQuery(drawer.slot,drawer.query,form)} t={t} timezone={timezone} busy={busy==="review-query"}/>:null}
...
function QueryReview({busy,detail,error,locale,onAdvancedEdit,onSubmit,t,timezone}:{
  ...
  return <form className="admin-drawer-form admin-query-review" onSubmit={(event)=>{event.preventDefault();onSubmit(new FormData(event.currentTarget));}}>
    ...
    <section className="admin-query-review__query"><header><div><h3>{t("review.proposal")}</h3><p>{detail.lineage.generated_at?t("review.generatedAt",{date:formatAdminInstant(detail.lineage.generated_at,locale,timezone)}):t("review.operatorVersion")}</p></div><button className="admin-button admin-button--compact" onClick={onAdvancedEdit} type="button"><PencilSimple aria-hidden size={14}/>{t("actions.advancedEdit")}</button></header><pre>{detail.query_text}</pre></section>
