...
export function SelfServiceImportManager({ brandId, workspaceId, timezone }: {
  ...
  return <>
    <AdminResourceSection title={t("title")} subtitle={t("body")}>
      ...
      {!loading && setup && (!configured || addingSource) ? <form className="admin-drawer-form" onSubmit={(event) => {
        ...
        <div><strong>{t(setup.ready_for_import ? "ready" : "storageBlocked.title")}</strong><p>{t(setup.ready_for_import ? "readyBody" : "storageBlocked.body")}</p></div>
    ...
    {setup?.ready_for_import ? <AcquisitionPlanManager key={revision} importsOnly preparedSourceKeys={setup.sources.map((source) => source.source_key)} timezone={setup.timezone || timezone} workspaceId={workspaceId} /> : null}
