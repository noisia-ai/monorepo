...
export function WorkspaceInterpretationAdmissionControls({ status, canAuthorize, canRevoke, submitting, pendingRequest = null, onSubmit }: {
  status: WorkspaceAnalysisStatus; canAuthorize: boolean; canRevoke: boolean; submitting: boolean;
  pendingRequest?: WorkspaceInterpretationAdmissionRequest | null;
  ...
  return <div className="admin-drawer-form" data-interpretation-admission={admission.current?.action ?? "none"}>
    <strong>{t("title")}</strong>
    <p className="admin-drawer-form__hint">{t("receipts", { confirmed: money(admission.confirmed_micro_usd), reserved: money(admission.reserved_micro_usd) })}
      {admission.terminal_reserved_micro_usd > 0 ? <> {t("terminal", { amount: money(admission.terminal_reserved_micro_usd) })}</> : null}</p>
    ...
    {admission.can_authorize && !pendingRequest ? <>
      ...
      <label className="admin-field"><span>{t("cap", { amount: money(admission.maximum_grant_micro_usd) })}</span>
      ...
      <p className="admin-drawer-form__hint">{t("limits", { run: money(admission.run_cap_micro_usd), daily: money(admission.daily_cap_micro_usd) })}</p>
      <p className="admin-drawer-form__hint">{t("partial")}</p>
      ...
      <button type="button" className="admin-button admin-button--primary" disabled={!enabled || submitting}
        onClick={() => { if (body && enabled) void onSubmit(body); }}>{t("authorize")}</button>
