...
import { AdminFeedbackState,AdminResourceSection,AdminSummaryStrip,
...
import { TopicRuleCohortManager } from "./TopicRuleCohortManager";
import { copySignalTopicEvaluationV2RefinementWording,createSignalTopicEvaluationV2ReviewIdempotencyKey,
  ...
  type SignalTopicEvaluationV2CandidateDetail,
...
export function FullEvidenceTopicCandidateManager({workspaceId,onImportedResult}:{workspaceId:string;
  ...
  return<FullEvidenceTopicCandidateManagerScope key={workspaceId} workspaceId={workspaceId} onImportedResult={onImportedResult}/>;
}
function FullEvidenceTopicCandidateManagerScope({workspaceId,onImportedResult}:{workspaceId:string;
  onImportedResult?:(imported:boolean)=>void}){
  ...
  const[cohortBusy,setCohortBusy]=useState(false),[cohortRefresh,setCohortRefresh]=useState(0);
  const refreshCohort=useCallback(()=>setCohortRefresh(value=>value+1),[]);
  ...
  async function open(candidateKey:string,event:MouseEvent<HTMLButtonElement>){
    if(!page?.run_key||busy||ruleBusy||cohortBusy)return;openerRef.current=event.currentTarget;setBusy(true);setError(null);
    try{const loaded=parseSignalTopicEvaluationV2CandidateDetail(await requestJson(
      `${endpoint}/${encodeURIComponent(candidateKey)}?run_key=${encodeURIComponent(page.run_key)}`));
  ...
  async function command(action:"save"|"reject"|"restore"|"undo"){
    ...
    try{await requestJson(`${endpoint}/${encodeURIComponent(candidate.candidate_key)}/commands`,{
      ...
      setDetail(null);refreshCohort();await load();
  ...
  return<><AdminResourceSection actions={<button className="admin-button" disabled={loading||busy||ruleBusy||cohortBusy}
      onClick={()=>{refreshCohort();void load();}} type="button">{loading?<CircleNotch aria-hidden className="icon--spin" size={14}/>:null}
    ...
    {page?<><AdminSummaryStrip density="compact" items={[
      ...
      {page.run_key?<TopicRuleCohortManager workspaceId={workspaceId} runKey={page.run_key} refreshVersion={cohortRefresh}
        editorBusy={busy||ruleBusy||!!detail} onBusyChange={setCohortBusy} onOpenCandidate={(key,event)=>void open(key,event)}/>
  ...
  {candidate&&detail?<WorkspaceDrawer ariaLabel={t("drawer.title")} closeLabel={t("actions.close")}
    ...
    <div className="admin-drawer-form topic-evaluation-manager__editor"><p className="admin-drawer-form__intro">{t("drawer.boundary")}</p>
      ...
      <TopicCandidateRuleDraft key={`${workspaceId}:${detail.run_key}:${candidate.candidate_key}`}
        ...
        editorBusy={busy} onBusyChange={setRuleBusy} onRefreshCandidate={refreshCandidate} onRuleSaved={refreshCohort}/>
