import { loadSignalSemanticContextReadinessProductV1 } from "@/lib/data-os/signal-semantic-context-pack";
...
export async function GET(_request:Request,context:{params:Promise<{workspaceId:string}>}){
  ...
  try{return semanticContextResponse(await loadSignalSemanticContextReadinessProductV1({
    workspace:loaded.workspace,actor:loaded.session.appUser}));}
  catch(error){return semanticContextError(error,"semantic_context_readiness_unavailable");}
}
