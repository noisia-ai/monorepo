...
import { topicCohortError,topicCohortResponse } from "@/lib/data-os/signal-topic-rule-cohort-api";
import { topicCohortTrialRequestSchema } from "@/lib/data-os/signal-topic-rule-cohort-management";
import { testTopicCohortProduct } from "@/lib/data-os/signal-topic-rule-cohort-product";
...
export async function POST(request:Request,context:Context){
  ...
  let body;try{body=topicCohortTrialRequestSchema.parse(await request.json());}catch{return topicCohortResponse({error:"topic_rule_cohort_request_invalid"},422);}
  if(body.run_key!==runKey)return topicCohortResponse({error:"topic_rule_cohort_scope_mismatch"},422);
  try{return topicCohortResponse(await testTopicCohortProduct({workspace:loaded.workspace,actor:loaded.session.appUser,runKey,request:body,idempotencyKey}));}
