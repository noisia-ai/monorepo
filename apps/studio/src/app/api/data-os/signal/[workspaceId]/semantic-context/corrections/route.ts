...
const signalSemanticContextCorrectionCommandV2=z.object({generation_key:key,element_key:key,
...
export async function POST(request:Request,context:{params:Promise<{workspaceId:string}>}){
  ...
  const parsed=signalSemanticContextCorrectionCommandV2.safeParse(await request.json().catch(()=>null));
