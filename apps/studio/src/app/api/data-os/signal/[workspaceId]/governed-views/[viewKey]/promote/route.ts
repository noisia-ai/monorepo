...
export async function POST(request: Request, context: { params: Promise<{ workspaceId: string; viewKey: string }> }) {
  ...
  if (!view.success || !body.success || key.length < 8 || key.length > 500) {
    return Response.json({ error: "invalid_governed_view_promotion", message: "A closed view, CAS digest and Idempotency-Key are required." }, { status: 422 });
  }
  try {
    const result = await transitionSignalGovernedViewProductV1({
      workspace: loaded.workspace, actor: loaded.session.appUser,
      viewKey: view.data, action: "promote", expectedSetDigest: body.data.expected_set_digest,
      idempotencyKey: key
    });
    return Response.json(result, { headers: { "Cache-Control": "private, no-store" } });
  } catch {
    return Response.json({ error: "governed_view_promotion_rejected", message: "The governed view could not be promoted from its current state." }, { status: 409 });
  }
}
