import { z } from "zod";
import { publishSignalSemanticContextGenerationProductV2,
  SIGNAL_SEMANTIC_CONTEXT_PUBLICATION_CONFIRMATION_V2 } from
...
const command=z.object({generation_key:z.string().regex(/^[a-z0-9]+(?:[._:-][a-z0-9]+)*$/u).max(200),
  ...
  confirmation:z.literal(SIGNAL_SEMANTIC_CONTEXT_PUBLICATION_CONFIRMATION_V2)}).strict();
export async function POST(request:Request,context:{params:Promise<{workspaceId:string}>}){
  ...
  try{return semanticContextResponse(await publishSignalSemanticContextGenerationProductV2({
    workspace:loaded.workspace,actor:loaded.session.appUser,idempotencyKey,
    generationKey:parsed.data.generation_key,preflightDigest:parsed.data.preflight_digest,
