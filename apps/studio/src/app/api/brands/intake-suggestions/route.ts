...
const BRAND_INTAKE_SUGGESTION_MODEL = "claude-sonnet-4-6";
const BRAND_INTAKE_SUGGESTION_MAX_OUTPUT_TOKENS = 4096;
const BRAND_INTAKE_SUGGESTION_MAX_WEB_SEARCHES = 2;
...
export async function POST(request: Request) {
  ...
  if (!internalCreation && !clientCreation.allowed) return forbidden();
  ...
  const model = BRAND_INTAKE_SUGGESTION_MODEL;
  ...
  return Response.json({
    ...
    model,
    limits: {
      max_output_tokens: BRAND_INTAKE_SUGGESTION_MAX_OUTPUT_TOKENS,
      max_web_searches: BRAND_INTAKE_SUGGESTION_MAX_WEB_SEARCHES
    },
    suggestions
...
async function generateWithClaude(input: z.infer<typeof bodySchema>, model: string): Promise<BrandIntakeSuggestions> {
  const timeoutMs = boundedInteger(process.env.ANTHROPIC_BRAND_OS_TIMEOUT_MS, 1_000, 150_000, 150_000);
  const maxSearches = boundedInteger(
    process.env.ANTHROPIC_BRAND_OS_WEB_SEARCH_MAX_USES,
    1,
    BRAND_INTAKE_SUGGESTION_MAX_WEB_SEARCHES,
    BRAND_INTAKE_SUGGESTION_MAX_WEB_SEARCHES
  );
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: boundedInteger(
        process.env.ANTHROPIC_BRAND_OS_MAX_TOKENS,
        512,
        BRAND_INTAKE_SUGGESTION_MAX_OUTPUT_TOKENS,
        BRAND_INTAKE_SUGGESTION_MAX_OUTPUT_TOKENS
      ),
      temperature: 0.1,
      system: [
        ...
        input.locale === "en-US"
          ? "Write every suggested field in professional English."
          : "Escribe todos los campos sugeridos en español profesional."
      ].join(" "),
      tools: [
...
function buildPrompt(input: z.infer<typeof bodySchema>) {
  return [
    ...
    "Reglas:",
    input.locale === "en-US"
      ? "- Write in professional, direct English in Noisia's strategy style."
...
function normalizeSuggestions(
  ...
  return {
    ...
    research_assumptions: uniqueList([
      ...
      ...(aliases.length < 6
        ? [input.locale === "en-US"
          ? "Aliases were supplemented with deterministic intake variants; validate exact handles before final queries."
...
function fallbackStrategicDescription(input: z.infer<typeof bodySchema>) {
  ...
  if (input.locale === "en-US") {
    return [
      `${input.display_name || input.brand} requires researched validation as a ${input.industry} brand`,
      input.subindustries.length > 0 ? `with an initial focus on ${input.subindustries.join(", ")}` : "",
      countryLabels ? `for ${countryLabels}` : "",
      "before opening corpus ingestion, queries, and analysis.",
      "This operational draft keeps Brand OS moving when external research is incomplete; validate it against the official site, public handles, actual competitors, first-party performance, and social listening before treating it as canonical context."
    ].filter(Boolean).join(" ");
  }
  return [
...
function fallbackKnowledgeNotes(input: z.infer<typeof bodySchema>) {
  ...
  if (input.locale === "en-US") {
    return [
      `- Initial intake: ${input.display_name || input.brand} / ${input.organization_name || input.brand}.`,
      `- Captured market: ${countryLabels || input.countries.join(", ")}.`,
      `- Initial taxonomy: ${input.industry}${input.subindustries.length ? ` / ${input.subindustries.join(", ")}` : ""}.`,
      "- Validate next: official site, sales channels, app, social profiles, public claims, recent news, and direct competitors.",
      "- Main research challenges: disambiguate mentions, separate generic category noise, map actual competitors, validate purchase barriers and triggers, and connect findings to Data OS.",
      "- This block is a traceable fallback and must not be treated as final research."
    ].join("\n");
  }
  return [
