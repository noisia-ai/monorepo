...
test("legacy Topic Evaluation stays available while Brand OS routes daily work to Topics", async () => {
  ...
  assert.match(topicsManager, /feedback\.archived/u);
  assert.match(topicsManager, /hasUnsupportedSignalScope/u);
  assert.match(topicsManager, /scopeNotice/u);
  assert.match(topicsApi, /loadSignalWorkspaceContextForTopics/u);
  ...
  assert.match(topicCommands, /expectedDefinitionRevision: command\.expected_definition_revision/u);
  assert.match(topicCommands, /expectedDefinitionDigest: command\.expected_definition_digest/u);
  ...
  assert.match(topicsStore, /replacementPending && execution\?\.status === "failed"/u);
  assert.doesNotMatch(topicsStore, /replacementPending && \(!execution/u);
  assert.match(topicsStore, /topic_signal_scope_unsupported/u);
  assert.match(topicsStore, /context_refs: refs/u);
  assert.match(topicsStore, /contextRows\(contextItems, 64\)/u);
  assert.doesNotMatch(topicsProduct, /semantic-replace|archive-replace/u);
  assert.match(topicsProduct, /correction-replace/u);
  assert.doesNotMatch(topicsManager, /X-Noisia-Embedding-Cost-Cap-Micro-Usd|EDIT_RECLASSIFICATION_CAP_MICRO_USD/u);
  assert.match(JSON.parse(es).AdminWorkspace.topics.feedback.archived, /archivado/u);
  assert.match(JSON.parse(en).AdminWorkspace.topics.feedback.archived, /archived/u);
