...
- La escritura se ensayó en PostgreSQL real privado, con datos sintéticos y rollback físico. El corte sigue **LOCAL**: no se ha aplicado SQL0185 ni desplegado el editor en UAT. Alexa+ continúa con 2/42 lotes y sin catálogo consolidado final, así que tampoco existe un caso real para pulsar el editor.
...
- El ensayo destapó SQLSTATE `42702` en `prepare_signal_topic_consolidation_snapshot_v1`: la variable PL/pgSQL `source` chocaba con una columna de CTE. SQL0185 renombra únicamente esa variable a `source_binding`; se ejecutó y verificó **dentro de la transacción revertida de dev-test**. Aún no está aplicado en UAT. No se modificaron tablas, permisos ni recibos.

## Siguiente acción

Aplicar SQL0185 una sola vez en UAT tras preflight de identidad y función, entregar el editor focal y comprobar la interfaz. La prueba real de editar/seleccionar una revisión consolidada requiere terminar Alexa+; con permiso de gasto vigente, reanudar la misma revisión (40 lotes), evaluar Noise/uniones/separaciones y entonces recorrer edición/selección en UI con datos reales.
