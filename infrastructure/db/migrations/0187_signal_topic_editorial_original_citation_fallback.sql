-- A malformed settled repair may leave the original as the only usable evidence.
-- Accept its deterministic citation quarantine only after both calls settled;
-- preserve every paid response, request identity and historical checkpoint.
