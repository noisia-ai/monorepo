-- Derive one explicit abstention from a complete, settled repair that omitted
-- exactly one sealed group. All other paid decisions remain byte-identical.
CREATE FUNCTION signal_topic_editorial_paid_missing_output_v1(output jsonb,receipts jsonb,batch_index integer)
 RETURNS jsonb LANGUAGE plpgsql IMMUTABLE SET search_path=public,pg_temp AS $$
DECLARE item jsonb;receipt jsonb;seen text[]:=ARRAY[]::text[];missing text:=NULL;
BEGIN
 IF output->>'contract_version'<>'signal-topic-editorial-screening-output-v1'
  OR output->>'batch_index' IS DISTINCT FROM batch_index::text
  OR (SELECT count(*) FROM jsonb_object_keys(output))<>3
  OR jsonb_typeof(output->'decisions')<>'array' OR jsonb_typeof(receipts)<>'array'
  OR jsonb_array_length(receipts) NOT BETWEEN 2 AND 40
  OR jsonb_array_length(output->'decisions')<>jsonb_array_length(receipts)-1 THEN RETURN NULL;END IF;
 FOR item IN SELECT value FROM jsonb_array_elements(output->'decisions') LOOP
  IF jsonb_typeof(item)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(item))<>6
   OR item->>'group_key' IS NULL OR item->>'group_key'=ANY(seen) THEN RETURN NULL;END IF;
  SELECT value INTO receipt FROM jsonb_array_elements(receipts) WHERE value->>'group_key'=item->>'group_key';
  IF receipt IS NULL OR jsonb_typeof(item->'cited_ref_ids')<>'array'
   OR jsonb_typeof(receipt->'evidence_ref_ids')<>'array' THEN RETURN NULL;END IF;
  IF EXISTS(SELECT 1 FROM jsonb_array_elements_text(item->'cited_ref_ids') cited(ref_id)
    WHERE NOT (receipt->'evidence_ref_ids') ? cited.ref_id) THEN RETURN NULL;END IF;
  seen:=array_append(seen,item->>'group_key');
 END LOOP;
 FOR receipt IN SELECT value FROM jsonb_array_elements(receipts) LOOP
  ...
  IF NOT ((receipt->>'group_key')=ANY(seen)) THEN
   IF missing IS NOT NULL THEN RETURN NULL;END IF;
   missing:=receipt->>'group_key';
  END IF;
 END LOOP;
 IF missing IS NULL THEN RETURN NULL;END IF;
 RETURN jsonb_set(output,'{decisions}',output->'decisions'||jsonb_build_array(jsonb_build_object(
  'group_key',missing,'disposition','unresolved','candidate',NULL,'confidence',NULL,
  'rationale','Respuesta pagada omitió este grupo; requiere nueva evidencia.','cited_ref_ids','[]'::jsonb)));
...
BEGIN
 ...
 WITH outputs AS MATERIALIZED (
  ...
  UNION ALL
  SELECT r.batch_index,signal_topic_editorial_normalize_output_v1(
    signal_topic_editorial_paid_missing_output_v1(c.response_output,r.receipts,r.batch_index)) AS normalized
   FROM requests r JOIN signal_topic_editorial_requests child ON child.parent_request_id=r.id
   JOIN signal_topic_editorial_calls c ON c.request_id=child.id
   WHERE child.execution_id=NEW.id AND child.workspace_id=NEW.workspace_id AND child.phase='screening'
    AND c.execution_id=NEW.id AND c.workspace_id=NEW.workspace_id AND c.status='settled'
    AND c.response_http_status=200 AND c.response_complete=true
    AND signal_topic_editorial_paid_missing_output_v1(c.response_output,r.receipts,r.batch_index) IS NOT NULL
 ...
 RETURN NEW;
END;$$;
REVOKE ALL ON FUNCTION signal_topic_editorial_paid_missing_output_v1(jsonb,jsonb,integer) FROM PUBLIC;
DO $$ DECLARE role_name text; BEGIN
 FOREACH role_name IN ARRAY ARRAY['anon','authenticated'] LOOP
  IF EXISTS(SELECT 1 FROM pg_roles WHERE rolname=role_name) THEN
   EXECUTE format('REVOKE ALL ON FUNCTION signal_topic_editorial_paid_missing_output_v1(jsonb,jsonb,integer) FROM %I',role_name);
  END IF;
 END LOOP;
END $$;
