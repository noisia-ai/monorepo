...
export type SignalWorkspaceEngineStatusV1 = {
  observed_at: string; workspace_id: string; latest_run: null | {
    ...
    phase: "queued" | "exporting" | "fitting" | "persisting" | "complete" | "failed";
...
async function current(client: PoolClient, run: Run, full: boolean) {
  ...
  if (full) {
    const plan = await loadSignalWorkspaceTopicPrototypePlanV1({queryable:client,workspace_id:run.workspace_id,actor_user_id:run.actor_user_id});
    if (plan.taxonomy_profile_id !== run.input_snapshot.taxonomy_profile_id || plan.plan_digest !== run.input_snapshot.prototype_plan_digest) return fail("workspace_engine_inputs_stale");
  }
}
...
async function buildInput(client: PoolClient, workspace: string, actor: string) {
  const plan = await loadSignalWorkspaceTopicPrototypePlanV1({queryable:client,workspace_id:workspace,actor_user_id:actor});
  const source = await loadSignalWorkspaceTopicInputSnapshotWithQueryableV1({queryable:client,workspace_id:workspace,actor_user_id:actor,allow_empty:true});
...
export async function beginSignalWorkspaceEngineV1(args:{database:SignalWorkspaceEngineDatabaseV1;workspace_id:string;actor_user_id:string;
  ...
  claude_cap_micro_usd:number;engine_config:Record<string,unknown>;parent_execution_id?:string|null}):Promise<{execution_id:string;replayed:boolean}> {
  ...
  const requestDigest=signalWorkspaceEmbeddingDigestV1({embedding_run_id:args.embedding_run_id,context_digest:args.expected_context_digest,
    catalog_digest:args.expected_catalog_digest,claude_cap_micro_usd:args.claude_cap_micro_usd,engine_config:args.engine_config,parent_selection:args.parent_execution_id===undefined?"latest-compatible-complete-v1":args.parent_execution_id});
...
export async function claimSignalWorkspaceEngineV1(args:{database:SignalWorkspaceEngineDatabaseV1;execution_id:string;worker_job_id:string}):Promise<SignalWorkspaceEngineLeaseV1|null>{
  return transaction(args.database,async client=>{const run=await lockedRun(client,args.execution_id);
    ...
    const token=randomUUID();await client.query(`UPDATE signal_topic_catalog_executions SET status='running',execution_token=$2::uuid,
      ...
      result_summary=result_summary||jsonb_build_object('phase','exporting','worker_job_id',$3::text),updated_at=clock_timestamp() WHERE id=$1::uuid`,[run.id,token,args.worker_job_id]);
...
export async function heartbeatSignalWorkspaceEngineV1(args:{database:SignalWorkspaceEngineDatabaseV1;lease:SignalWorkspaceEngineLeaseV1;
  phase?:'exporting'|'fitting'|'persisting';exported?:{roots:number;chunks:number;guides:number;stream_digest:string}}):Promise<void>{
...
export async function persistSignalWorkspaceEngineArtifactV1(args:{database:SignalWorkspaceEngineDatabaseV1;lease:SignalWorkspaceEngineLeaseV1;
  ...
  const a=args.artifact,prefix=`workspace-engine/${args.lease.workspace_id}/${args.lease.execution_id}/`;
...
export async function finishSignalWorkspaceEngineFitV1(args:{database:SignalWorkspaceEngineDatabaseV1;lease:SignalWorkspaceEngineLeaseV1;
  ...
  model_configuration:Record<string,unknown>;runtime_kind:string;artifact_format:string;license_key:string|null}):Promise<{execution_id:string;model_version_id:string|null}>{
  return transaction(args.database,async client=>{const run=await requireLease(client,args.lease,true);
    ...
    if(args.result_kind==='insufficient_population') {
      const result={...run.result_summary,phase:'complete',model_version_id:null,model_artifact_id:null,output_artifact_id:output.id,
        coverage:args.coverage,semantic_approval:'none',result_kind:'insufficient_population'};
      await client.query(`UPDATE signal_topic_catalog_executions SET status='ready',progress=100,result_summary=$2::jsonb,
        execution_token=NULL,execution_expires_at=NULL,completed_at=clock_timestamp(),updated_at=clock_timestamp() WHERE id=$1::uuid`,[run.id,JSON.stringify(result)]);
      await completeDispatch(client,run.id);
      return {execution_id:run.id,model_version_id:null};
    }
    if(!model)return fail('workspace_engine_artifacts_incomplete');
    ...
    const result={...run.result_summary,phase:'complete',model_version_id:registered.model_version_id,model_artifact_id:model.id,
      output_artifact_id:output.id,coverage:args.coverage,semantic_approval:'none',result_kind:'computational_grouping'};
    await client.query(`UPDATE signal_topic_catalog_executions SET status='ready',progress=100,result_summary=$2::jsonb,
      execution_token=NULL,execution_expires_at=NULL,completed_at=clock_timestamp(),updated_at=clock_timestamp() WHERE id=$1::uuid`,[run.id,JSON.stringify(result)]);
    await completeDispatch(client,run.id);
    return{execution_id:run.id,model_version_id:registered.model_version_id};
  });
}
...
export async function loadSignalWorkspaceEngineStatusV1(args:{database:SignalWorkspaceEngineDatabaseV1;workspace_id:string;actor_user_id:string;
  ...
  return transaction(args.database,async client=>{
    ...
    if(rows.length){try{planDigest=(await loadSignalWorkspaceTopicPrototypePlanV1({queryable:client,workspace_id:args.workspace_id,actor_user_id:args.actor_user_id})).plan_digest;}
    ...
    const view=async(row:typeof rows[number]):Promise<NonNullable<SignalWorkspaceEngineStatusV1['latest_run']>>=>{
      ...
      return{execution_id:row.id,status:row.status,phase:row.result_summary.phase as NonNullable<SignalWorkspaceEngineStatusV1['latest_run']>['phase'],
        ...
        is_current:row.revision_live&&row.policy_live&&planDigest===row.input_snapshot.prototype_plan_digest&&actorCanExecute,
