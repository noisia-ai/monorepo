...
const report={contract_version:'interest-preparation-sql-preflight-receipt-v1',acceptance_scope:'ddl_contracts_negative_paths_only',full_preparation_acceptance:false,status:'blocked',remote_connected:false,remote_executed:false,fixture_mutations_started:false,
 migration_installed_in_transaction:false,provider_transports:0,assertions:[],physical_rollback:false,post_rollback_empty:false};
...
try{
 ...
 pool=new pg.Pool({...connection,host:addresses[0].address,ssl:false,max:1,connectionTimeoutMillis:10_000,
  application_name:'interest-preparation-private-synthetic-test',statement_timeout:30_000,idle_in_transaction_session_timeout:45_000});
 ...
 // Exclude writes and DDL while rehearsing a rollback-only migration.
 ...
 await assertInterestPreparationBase(client);
 ...
 await client.query(migration.toString('utf8'));report.migration_installed_in_transaction=true;
 await publicTables(client,manifest.installed_table_count);
 ...
 report.table_count=tableCount;report.migration_sha256=manifest.migration_sha256;report.schema_sha256=beforeSchema;report.status='passed';
}catch(error){report.error_code=fixedError(error);report.status=report.migration_installed_in_transaction||report.fixture_mutations_started?'failed':'blocked';
